...
public class Lock extends BaseLock{
    ...
    public Boolean lock(boolean block, Long timeInterval, Integer maxTimes) throws InterruptedException {
        ...
        while(true) {
            ...
            if(consulClient.setKVValue(keyPath, "lock:" + DateTime.now(), putParams).getValue()) {
                return true;
            } else if(block) {
    ...
    public Boolean unlock() {
        ...
        boolean result = consulClient.setKVValue(keyPath, "unlock:" + DateTime.now(), putParams).getValue();
